...
(function squireClassInit(doc) {
 ...
 // If you delete the content inside a span with a font styling, Webkit will
    ...
    var keyHandlers = {
        enter: function(self, event, range) {
            ...
            while (nodeAfterSplit.nodeType === ELEMENT_NODE) {
                ...
                if (nodeAfterSplit.nodeName === 'A' &&
                    (!nodeAfterSplit.textContent ||
                        nodeAfterSplit.textContent === ZWS) && nodeAfterSplit.getElementsByTagName("img").length == 0) {
            ...
            self.setSelection(range);
            self._updatePath(range, true);
            /* zoho desk - scroll to view */
            // Scroll into view
            if (nodeAfterSplit.nodeType === TEXT_NODE) {
                nodeAfterSplit = nodeAfterSplit.parentNode;
            }
            // 16 ~ one standard line height in px.
            if (nodeAfterSplit.getBoundingClientRect().top > self._editorRef().iframe.offsetHeight) {
                nodeAfterSplit.scrollIntoView( false );
                var editor = self._editorRef();
                if(editor.initobj.needScrollView){
                    var ua = navigator.userAgent;
                    var elem = (ua.indexOf("Firefox") != -1 || ua.indexOf("MSIE") != -1 || !!document.documentMode == true ) ? document.documentElement : document.body;
                    elem.scrollTop = editor.toolbardiv.getBoundingClientRect().top;
                }
            }
        },
        ...
        tab: function(self, event, range) {
            ...
            else if(self._doc.queryCommandState("insertOrderedList") || self._doc.queryCommandState("insertUnOrderedList")){
                event.preventDefault();
                self.increaseListLevel(range);
            }
        },
    ...
    // thing.
    // bugzilla.mozilla.org/show_bug.cgi?id=289384
    if (isMac && isGecko) {
    ...
    */
    ...
    var onCut = function(event) {
        ...
        // bugs.webkit.org/show_bug.cgi?id=143776
    ...
    var onPaste = function(event) {
        ...
        if ( items ) {
            event.preventDefault();
            l = items.length;
            /* zoho desk - code check error */
            var insertHTMLFromPaste = function ( html ) {
            ...
            while (l--) {
                ...
                if (!choosePlain && type === 'text/html') {
                    ...
                    item.getAsString(insertHTMLFromPaste);
         ...
         // Treat image paste as a drop of an image file.
            if (hasImage) {
                ...
                if (fireDrop) {
                    this.fireEvent('drop', {
                        dataTransfer: clipboardData,
                        editorCustomPaste: true /* zoho desk - find custom paste */
                    });
                }
            } else if (plainItem) {
        ...
        // bugzilla.mozilla.org/show_bug.cgi?id=1254028
    ...
    function Squire(root, config) {
        ...
        this._events = {};
        if (config && config.getEditorInstance) {
            this._editorRef = config.getEditorInstance;
        }
        ...
        //this.setHTML( '' ); /* zoho desk - default block*/
    }
    ...
    var sanitizeToDOMFragment = function(html, isPaste, self) {
        var doc = self._doc;
        var nPurify = self._editorRef().NewDOMPurify; /*zoho desk - new dom purify added */
        var frag = html ? nPurify.sanitize(html, {
    ...
    /*zoho desk -  after paste the url like content - convert to link format*/
    proto.addTextLinks = function(startcontainer, root, self){
        addLinks(startcontainer, root, self);
    };
    ...
    proto.didError = function(error) {}; /*zoho desk - code check error*/
    ...
    // WebKit bug: bugs.webkit.org/show_bug.cgi?id=15256
    ...
    /*zoho desk - For toc we need to support h1,h2,h3*/
    var tagAfterSplit = {
        DT: 'DD',
        DD: 'DT',
        LI: 'LI',
    ...
    var splitBlock = function(self, block, node, offset) {
        ...
        /*zoho desk - for TOC*/
        if(["H1","H2","H3"].indexOf(nodeAfterSplit.nodeName) >= 0 && nodeAfterSplit.innerText.trim().length < 1){
            nodeAfterSplit.removeAttribute("id");
        }
    ...
    proto._setHTML = function(html) {
        ...
        var node = root;
        node.innerHTML = html;
        /* zoho desk - Img Table max width added */
        var addMaxWidth = function(tags){
            var tagLength = tags && tags.length;
            if(tags){
        ...
        addMaxWidth(node.getElementsByTagName('img'));
        addMaxWidth(node.getElementsByTagName('table'));
        do {
            fixCursor(node, root);
        } while (node = getNextBlock(node, root));
        this._ignoreChange = true;
    ...
    proto.setHTML = function(html) {
        ...
        addMaxWidth(div.getElementsByTagName('img'));
        addMaxWidth(div.getElementsByTagName('table'));
        if (typeof sanitizeToDOMFragment === 'function') {
            frag = sanitizeToDOMFragment(div.innerHTML, false, this);
        } else {
    ...
    /*zoho desk - extract common container for text styles*/
    proto. extractCommonContainer = function(range, common, root){
        return extractContentsOfRange(range, common, root);
    }
    ...
    proto.insertHTML = function(html, isPaste) {
        ...
        // available use NewDOMPurify to parse and sanitise.
        if (typeof sanitizeToDOMFragment === 'function') {
            ...
            div = this.createElement('DIV');
            div.innerHTML = html;
            /*zoho desk- on paste - img tags adding default style*/
            var addMaxWidth = function(tags){
                var tagLength = tags && tags.length;
                if(tags){
                    for(var i = 0; i < tagLength; i++){
        ...
        /* zoho desk- for TOC */
        this._editorRef().initobj.styletext && this._editorRef().insertTOC();
        return this;
    };
    ...
    proto.setFontSize = function(size) {
        var styleAttribute = undefined;
        styleAttribute = size > "15px" ? "font-size:"+size+";line-height:normal" : "font-size:"+size;
    ...
    /* zoho desk - class name, attributes removed*/
    ...
    /*Zoho desk - style attr for direction*/
 ...
 // Node.js exports
    ...
    if (typeof exports === 'object') {
        editorModule.exports = Squire; /* zoho desk - module obj - duplicate name collision on our pdt*/
